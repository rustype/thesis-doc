%!TEX root = ../template.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Config/7_package.tex
%% NOVA thesis configuration file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\typeout{NT FILE Config/7_packages.tex}

%%============================================================
%%  ADD YOUR PACKAGES AND OTHER CUSTOMIZATIONS HERE
%%============================================================
\usepackage{paralist}     % To enable costumizble enumerates.  See the documentation.

\usepackage[textsize=footnotesize]{todonotes}  % To register TODO notes in the text

\usepackage{makecell}      % Multilie table cells and control vertical and horizontal alignment

% \usepackage{indentfirst}       % Indent first list after a (sub)section title

\usepackage{inconsolata}

\usepackage[newfloat]{minted}
\setminted{
    linenos,
    frame=single,
    style=lovelace,
    fontsize=\small
}
\renewcommand{\theFancyVerbLine}{\small{\ttfamily{\arabic{FancyVerbLine}}}}

\usepackage[
    margin=0.5cm,
    position=bottom
]{caption}

\captionsetup[listing]{skip=0mm}

\usepackage{csquotes}
\usepackage{titlesec}
\usepackage{amssymb}
\usepackage{multirow}

\usetikzlibrary{automata}
\usetikzlibrary{positioning}
\usetikzlibrary{arrows.meta}
\usetikzlibrary{shapes.geometric}

\definecolor{stringred}{RGB}{184, 56, 56}
\definecolor{attrgreen}{RGB}{41, 153, 112}
\definecolor{structblue}{RGB}{41, 112, 135}

\newcommand{\ttt}[1]{\texttt{#1}}

\newcommand{\wrt}{w.r.t.}
\newcommand{\keyword}[1]{\textcolor{violet}{\texttt{#1}}}
\newcommand{\quotes}[1]{“{#1}”}
\newcommand{\equotes}[1]{“\emph{#1}”}
\newcommand{\annotation}[1]{\texttt{\#[#1]}}
\newcommand{\ie}[1]{(i.e.~#1)}
\newcommand{\eg}[1]{(e.g.~#1)}
\renewcommand{\citeurl}[2]{\footnote{\url{#1} (visited~in~#2)}}

% vertically center a table cell
% https://tex.stackexchange.com/a/473626
\newcommand{\centered}[1]{\begin{tabular}{l} #1 \end{tabular}}

% LaTeX hackery
\AtBeginDocument{%
  \addto\extrasenglish{%
    \renewcommand{\chapterautorefname}{Chapter}%
    \renewcommand{\sectionautorefname}{Section}%
    \renewcommand{\subsectionautorefname}{Section}%
    \renewcommand{\subsubsectionautorefname}{Section}%
  }
  % \newcommand{\listingautorefname}{Listing}%
}

% Make the citation end with " - <citation>"
\renewcommand*{\mkcitation}[1]{~---~#1}
\renewcommand{\mkbegdispquote}[2]{\itshape}

\titlespacing\paragraph{0pt}{*}{*}