...
\annotation{typestate} was designed with typestates and API constraints in mind,
...
while \annotation{typestate} enforces constraints on the nodes \ie{\quotes{internal} protocol},
...
Both \ttt{RingA} and \ttt{RingB} receive two channels
...
the channels are received upon instantiation of the respective \ttt{Ring} object.
...
Inversly to \ttt{RingA}, \ttt{RingB} will first receive the value,
...
The main difference from \ttt{RingA} to \ttt{RingB} is the initial state, the inner logic is the same
...
for both \ttt{B} and \ttt{C} (lines 7-8 of \autoref{lst:typestate-ring-main}).

\subsection{Summary}

As previously stated, Rumpsteak allows the developer to enforce communication patterns at compile-time,
while \annotation{typestate} does not, however, the latter is able to enforce behavior patterns at the participant level;
\annotation{typestate} is thus more flexible, being usable in synchronous or parallel programming,
...
For \annotation{typestate}, for example, one can define a macro like \autoref{lst:typestate-ring-macro} and
...
fn main() {
    ...
    \end{minted}
    \caption{The \ttt{main} implementing for \annotation{typestate}'s ring.}
...
macro_rules! spawn_ring_b {
    ...
    \end{minted}
    \caption{\ttt{macro\_rules!} to abstract over the thread launching routing for \ttt{RingB}.}
...
fn main() {
    ...
    \end{minted}
    \caption{The \ttt{main} implementing for \annotation{typestate}'s ring using \autoref{lst:typestate-ring-macro}.}
...
The state embedding also guarantees that the \ttt{Reader} is a well-behaved client of the \ttt{Card},
similar to Fugue \autocite{DeLine2004}.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Chapters/Figures/C5/reader_typestates.pdf}
    \caption{The \ttt{Reader} and \ttt{Card} typestates, the green arrow indicates the dependency relationship between states.}
...
In this chapter I have presented three examples which highlight different capabilities of the \annotation{typestate} macro.
The first example (\autoref{sec:case-study:ring}) shows how \annotation{typestate} is able to constrain the behavior of objects in multi threaded environments;
...
finally, the final example (\autoref{sec:case-study:auction}) illustrates how one can wrap an existing API using \annotation{typestate}
to provide a safe-to-use API.

While in some cases one can further abstract over the code written using \annotation{typestate},
such cases are often trivial to address using Rust's \ttt{macro\_rules!}.
Overall, \annotation{typestate} is able to address the problem at hand and
