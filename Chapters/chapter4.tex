...
This chapter presents the contributions of the present work, the \annotation{typestate} macro.
...
\paragraph{The \annotation{state} annotation,} like the \annotation{automaton} annotation, is only attachable to structures (\autoref{lst:typestate-mod} lines 6-8);
...
as demonstrated in the previous section, the DSL's entry point is the \annotation{typestate} attribute,
...
enum FallibleOperationResult {
    ...
    \end{minted}
    \caption{Fallible operations can be using enumerations like the \ttt{FallibleOperationResult} decision state.}
    \label{lst:typestate-decision}
\end{listing}

\paragraph{Transition labels} can be added through the use of the \ttt{metadata} attribute,
...
enum FallibleOperationResult {
    ...
    \end{minted}
    \caption{\autoref{lst:typestate-decision} enumeration with the \ttt{metadata} attribute.}
...
and the \ttt{aquamarine} crate\footnote{\url{https://docs.rs/aquamarine/0.1.9/aquamarine}} (detailed in \autoref{sec:automata-visualization:documentation-generation}).
